...
export const PendingItems: React.FC<PendingItemsProps> = ({ kpis, role }) => {
  ...
  const items = [
    ...
    },
    {
      ...
      icon: <ArrowRightLeft className="h-4 w-4 text-brand-blue" />,
    ...
    <Card className="rounded-2xl border border-slate-200 bg-white h-full flex flex-col">
      <CardHeader className="p-5 border-b border-slate-100 flex-shrink-0">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-bold text-primary">Pending Items</CardTitle>
          {totalPending > 0 && (
              ...
              (item) =>
                item.count > 0 && (
                  <Link
                    key={item.label}
                    to={item.to}
                    className="group block p-3.5 rounded-xl border border-slate-200 bg-white hover:border-brand-blue hover:shadow-sm transition-all duration-150"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-slate-700 group-hover:text-brand-blue transition-colors truncate">
                            {item.label}
                          </p>
                      ...
                      <div className="flex items-center gap-2 shrink-0 ml-3">
                        <span className="text-xs font-extrabold text-primary bg-slate-100 px-2 py-0.5 rounded-md">
                          {item.count}
                        </span>
                        <ArrowUpRight className="h-3.5 w-3.5 text-slate-400 group-hover:text-brand-blue group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all" />
                      </div>
                    </div>
                  </Link>
                )
